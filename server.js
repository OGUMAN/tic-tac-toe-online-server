...
const disconnected = (playerRoom, room, opponentRoom) => {
  if(toMenuIntervals?.get(room)?.func===undefined){ // checking is there already menu timer to avoid showing disconnect modal with another modal
    io.to(room).emit("disconnected"); // send to room that opponent left
    setToMenuTimer(room, 10); // set timer to send player to menu
    try { // does this player has timer in general?
      clearInterval(stepIntervals.get(playerRoom).func);
    } 
    catch { // if not
      try { // does this player has timer in general too?
...
io.on("connection", (socket) => {
  ...
  socket.on("im waiting", () => { // when player have rendered the component
    socket.join("in waiting"); // send player to waiting room
    if (roomLength("in waiting") > 1) { // when there are 2 players or more
  ...
  socket.on("disconnect", () => { // when any player disconnected
    io.to("in waiting").emit("send online", roomLength("im online")); //send number of online players to waiting room
  });
  socket.on("im leaving", (playerRoom, room, opponentRoom) => { // when player left the game
    socket.leave(room);
    socket.leave(playerRoom);
    socket.leave("in waiting");
    disconnected(playerRoom, room, opponentRoom);
  });
  socket.on("cellClick", (cells, id, room, step, whoClicked, whoWill) => { // when clicked any cell
    if (cells[id] === "") { // is the cell empty?
      let field = cells;
      let winnerCells = [];
      ...
      cellsCheck: for (let k = 0; k <= 4; k++) { // checking cells for a winner or when it's draw (using loop, to break it when winner found or it's draw)
        if (k === 0) { // horizontal checking
          let stepsInLine;
          for (let k = 0; k < cellsNumber; k += cellsInLine) { // checking every horizontal line
            stepsInLine = 0;
            winnerCells = [];
            for (j = k; j < k + cellsInLine; j++) { // checking every cell in the horizontal line
              if (cells[j] === step) { // if cell value = step
                stepsInLine++; // increment number of cells whit this value
                winnerCells.push(j); // push id to an array to make cross this cells
                if (stepsInLine === cellsInLine) { // if each line's cell has step value
        ...
        if (k === 1) { // vertical checking
          let stepsInLine;
          for (let k = 0; k < cellsInLine; k++) { // checking every vertical line
            stepsInLine = 0;
            winnerCells = [];
            for (j = k; j <= cellsNumber; j += cellsInLine) { // checking every cell in the vertical line
              if (cells[j] === step) { // if cell value = step
                stepsInLine++; // increment number of cells whit this value
                winnerCells.push(j); // push id to an array to make cross this cells
                if (stepsInLine === cellsInLine) { // if each line's cell has step value
        ...
        if (k === 2) { // diagonal checking from left top
          let stepsInLine = 0;
          winnerCells = [];
          for (let k = 0; k < cellsNumber; ) { // checking diagonal line from left top to right bottom (\)
            if (cells[k] === step) { // if cell value = step
              stepsInLine++; // increment number of cells whit this value
        ...
        if (k === 3) { // diagonal checking from left bottom
          let stepsInLine = 0;
          winnerCells = [];
          for (let k = cellsInLine - 1; k <= cellsNumber - 2; ) { // checking diagonal line from left bottom to right top (/)
            if (cells[k] === step) { // if cell value = step
              stepsInLine++; // increment number of cells whit this value
              winnerCells.push(k); // push id to an array to make cross this cells
              if (stepsInLine === cellsInLine) { // if each line's cell has step value
        ...
        if (k === 4) { // checking for a draw
          let busyCells = 0;
          for (j = 0; j <= cellsInLine * cellsInLine; j++) { // checking each cell of field
            if (cells[j] === "x" || cells[j] === "o") { // if cell has any value
              busyCells++; // increase the number of busy cells
            }
            if (busyCells === 9) { // if busy cells = 9
  ...
  socket.on("play again", (room, againVotes, myRoom) => { // when anyone clicked to play again 
    let againVotesIncremented = againVotes;
    if (againVotes < 2) { // if votes < 2
  ...
  socket.on("send name", (myName, opponentRoom) => { // when player sending his name
    io.to(opponentRoom).emit("get opponent name", myName); // send to opponent his name
  });
  socket.on("send message", (inputValue, myRoom, opponentRoom) => { // when player sending message
    ...
    const current =
      ...
      (now.getMinutes() < 10 ? "0" : "") +
      now.getMinutes();
    io.to(myRoom).emit("get message", { // send to player that send message it as his
      text: inputValue,
      time: current,
      whose: "my",
    });
    io.to(opponentRoom).emit("get message", { // send to player that send message it as opponent's
      text: inputValue,
      time: current,
      whose: "opponent",
    });
  });
  socket.on("get online", () => { // when player requests number of players online
